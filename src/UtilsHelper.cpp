...
string printSourceCodeInfo(Value *V) {
  ...
  } else if (FN.find("include/") != string::npos) {
    FN = FN.substr(FN.find("include/"));
  } else {
  }
...
void printSourceCodeInfoWithValue(Value *V) {
  ...
  while (line[0] == ' ' || line[0] == '\t')
    line.erase(line.begin());
  DEBUG_WITH_TYPE("statistics", dbgs() << "[Code] " << FN << " +" << LineNo << ": " << line);
}
    ...
    if (!curConst.isLogicAnd() && !curConst.isLogicOr() &&
      ...
      if (elements.size() == 1) {
        ...
        if (std::find(isRelevant.begin(), isRelevant.end(), elem) !=
          ...
          DEBUG_WITH_TYPE("condition", dbgs() << "Add relevant " << curConst.getSymbol() << "\n");
        }
        continue;
      }
    }
      ...
      if (has_relavent) {
        for (int i = 0; i < curConst->childItems.size(); i++) {
          if (curConst->childItems[i]->is_removed) {
            ...
            changedSize += 1;
            DEBUG_WITH_TYPE("condition", dbgs() << "Add into relavent "
      ...
      if (curConst->is_removed) {
        ...
        if (elements.size() == 1) {
          ...
          if (std::find(isRelevant.begin(), isRelevant.end(), elem) !=
            ...
            changedSize += 1;
           DEBUG_WITH_TYPE("condition",  dbgs() << "Add relevant " << curConst->curExpr.getSymbol() << "\n");
          }
        }
      }
    }
  ...
  ConstExpr *negated =
      negateConstraint(filtered
                           ->toExpr(SEGSolver, &SEGSolver->ExprStr2Operand,
                                    &SEGSolver->ExprStr2Opcode)
                           .toAndExpr());
  DEBUG_WITH_TYPE("condition", dbgs() << "Negated "
         << negated
                ->toExpr(SEGSolver, &SEGSolver->ExprStr2Operand,
                         &SEGSolver->ExprStr2Opcode)
                .toAndExpr()
