...
public:
  ...
  bool isSource(SEGNodeBase *Node, SEGSiteBase *Site) const {
    ...
    if (inputNode->type == ArgRetOfAPI && isa<SEGCallSiteCommonOutputNode>(Node)) {
...
class SingleSrcSingleSink : public SrcMustNotReachSinkVulnerability {
  ...
  ConstExpr *bugConstraint;

  unordered_map<std::string, const SEGNodeBase *> *expr2Operand;
  unordered_map<std::string, const SEGOpcodeNode *> *expr2Opcode;
...
public:
  ...
  SingleSrcSingleSink(const char *checkerName, GraphDiffer *graphParser,
      ...
      : SrcMustNotReachSinkVulnerability(checkerName),
        ...
        expr2Operand(&graphParser->SEGSolver->ExprStr2Operand),
        expr2Opcode(&graphParser->SEGSolver->ExprStr2Opcode) {
  ...
  virtual void setPrerequisites(SymbolicExprGraphSolver *Solver,
    ...
    if (isSource((SEGNodeBase *)TraceHistory.sourceNode(),
      ...
      Prerequisites.mergeWithAnd(
          bugConstraint->toExpr(Solver, expr2Operand, expr2Opcode));
    }
  }
