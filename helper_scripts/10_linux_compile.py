...
def rename_driverbcs(driver_bcs, linux_commit, arch):
    ...
    target_dir = os.path.abspath(BCs_DIR_12.format(linux_commit, arch))
...
def translate_36(linux_commit, arch):
    src_dir = os.path.abspath(BCs_DIR_12.format(linux_commit, arch))
    target_dir = os.path.abspath(BCs_DIR_3.format(linux_commit, arch))
    ...
    for root, dirs, files in os.walk(src_dir):
        ...
        for file in files:
            ...
            if not file_path.endswith('.bc'):
                continue
            driver_sub = os.path.basename(file_path).replace('.bc', '_36.bc')
            print('Translating to 3.6 ', driver_sub)

            if not os.path.exists(os.path.join(target_dir, driver_sub)):
                cmd_trans = "{} {} {}".format(TRANSLATOR, file_path, os.path.join(target_dir, driver_sub))
                status, output, error = run_cmd(src_dir, cmd_trans)
                if status == 0 and os.path.exists(os.path.join(target_dir, driver_sub)):
                    print('Step 3: Successfully Translated ', driver_sub)
                else:
                    print('Error ', error)
...
if __name__ == "__main__":
    for commit in LINUX_COMMIT.split(", "):
        ...
        for arch in ARCHS.split(", "):
            ...
            translate_36(linux_commit=commit, arch=arch)
