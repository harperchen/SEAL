#include "include/PatchParser.h"
#include "include/UtilsHelper.h"
#include "include/ValueHelper.h"

static cl::opt<bool, false>
    DumpDiffProcess("dump-ir-diff", cl::desc("Dump the process of IR difference."),
...
void PatchParser::parseIRChanges() {
  ...
  // make sure matchedIRsBefore and matchedIRAfter have the same number of mappings
  for (auto [beforeIR, afterIR] : matchedIRsBefore) {
...
// for each function in module, compute their start and end source code line number
...
void PatchParser::matchBBByLine() {
  ...
  for (const auto &entry : blockLineScope) {
    ...
    if (block->getParent()->getName().startswith("after.patch")) {
      ...
      //         dbgs() << "\nSrc file: " << src_file << "\n";
      //         dbgs() << "Range of original after bb " << block->getName() << " in "
      ...
      if (unChangedMapping.count(src_file)) {
        if (unChangedMapping[src_file].count(entry.second.first) &&
          ...
          //           if (changedFuncs.count(block->getParent())) {
          //             dbgs() << "Range of mapped after bb " << block->getName() << " in "
          //                    << block->getParent()->getName() << " is from "
...
//        DEBUG_WITH_TYPE("statistics", dbgs() << "Range of mapped from " << item.first.first << " to "
//               << item.first.second << ": " << pairedBlocks.size()
//               << "\nbb name:");
//        for (auto block : pairedBlocks) {
//          DEBUG_WITH_TYPE("statistics", dbgs() << " " << block->getName());
//        }
      ...
      if (pairedBlocks.size() >= 2) {
        ...
        for (auto bbBefore: pairedBlocks) {
          if (changedFuncs.count(bbBefore->getParent())) {
            DEBUG_WITH_TYPE("statistics", dbgs() << "Line Scope UnMatched Before BB:" << bbBefore->getName() << "\n");
          }
        }
      ...
      for (auto bbBefore : beforeBBs) {
        for (auto bbAfter : afterBBs) {
          ...
          if (isTwoValueMatchedHelper(bbBefore, bbAfter)) {
            if (changedFuncs.count(bbBefore->getParent())) {
              DEBUG_WITH_TYPE("statistics", dbgs() << "Line Scope Matched BB:" << bbBefore->getName() << ", "
                     << bbAfter->getName() << "\n");
            }
      ...
      for (auto bbAfter : afterBBs) {
        if (!matchedIRsAfter.count(bbAfter)) {
          if (changedFuncs.count(bbAfter->getParent())) {
            DEBUG_WITH_TYPE("statistics", dbgs() << "Line Scope UnMatched After BB:" << bbAfter->getName() << "\n");
          }
...
void PatchParser::matchUnChangedIRs() {
  ...
  for (auto [srcfile, lineNumMap] : unChangedMapping) {
    for (auto [beforeLine, afterLine] : lineNumMap) {
          ...
          if (DumpDiffProcess.getValue()) {
            for (auto afterIR : afterIRs) {
              if (matchedIRsAfter.find(afterIR) == matchedIRsAfter.end()) {
                ...
                dbgs() << "\t" << isTwoIRMatched(beforeIR, afterIR, true) << "\n";
              }
            }
          }
        }
      ...
      is_print = false;
      for (auto afterIR : afterIRs) {
        if (matchedIRsAfter.find(afterIR) == matchedIRsAfter.end() && !isCurrentIRSkipMatch(afterIR)) {
          if (!is_print) {
...
void PatchParser::getLineNumInsts(Function *func, int line,
  ...
  for (BasicBlock &bb : *func) {
    ...
    // for instance, the number of current instruction maybe larger than the next inst
    int last_line = -1;
...
unsigned int PatchParser::guessInstructionLineNum(Instruction* inst) {
  ...
  if (auto *icmpInst = dyn_cast<ICmpInst>(inst)) {
    for (auto use_it = icmpInst->user_begin();use_it != icmpInst->user_end(); use_it++) {
...
//          dbgs() << "Branch inst: " << next_ir_line << ", " << *next_inst << "\n";
          return next_ir_line;
        }
      }
    }
