...
class PatchParser:
    ...
    def parse_commit(self, workdir, item, is_failed):
        ...
        peer_num = 0
        # if not os.path.exists(os.path.join(workdir, 'peer.txt')):
        #     peer_num, ret = self.find_peer(workdir, indirect_call, item['arch'])
        #     if not ret:
        #         return [hexsha, workdir, True, True, True, True, False, 0, '', '', 'Target BC Failed', ' '.join(reason1 + reason2 + reason3)]
        # else:
        #     with open(os.path.join(workdir, 'peer.txt'), 'r') as f:
        #         line = f.read()
        #         peer_num = len(line.split())
    ...
    def compile_and_link(self, hexsha, cur_commit, arch, workdir, related_files, curtype):
        ...
        if not os.path.exists(link_target):
            link_cmd = "llvm-link-12 -v -o " + link_target + " " + " ".join(link_bcs)
            status, _, error = run_cmd(linux_dir, link_cmd)
            if status != 0:
                reasons.append(f'Error when link {curtype} ' + link_cmd)
                return False, reasons, found_commit

        file_root, _ = os.path.splitext(link_target)
        translate_target = file_root + '_36.bc'
        if not os.path.exists(translate_target):
            cmd = [TRANSLATOR, link_target, translate_target]
            status, _, _ = run_cmd(os.getcwd(), ' '.join(cmd))
            if status != 0:
                reasons.append(f'Error when translate {curtype} ' + ' '.join(cmd))
                return False, reasons, found_commit
    ...
    def merge_bc(self, dir_name):
        reasons = []
        if not os.path.exists(os.path.join(dir_name, 'after_36.bc')) or \
                not os.path.exists(os.path.join(dir_name, 'before_36.bc')):
            return False, reasons
        if not os.path.exists(os.path.join(dir_name, 'merge.bc')):
            cmd = MERGER + ' -after-bc=after_36.bc -before-bc=before_36.bc -o merge.bc'
    ...
    def detect_bugs(self, workdir, indirect_call):
        ...
        peer_file = os.path.join(workdir, 'peer.txt')
        report_file = os.path.join(workdir, 'bug.json')
        input_bc = os.path.join(workdir, 'merge.bc')
        log_file = os.path.join(workdir, 'patch.log')
        checker_cmd = SEALBIN + (" -ps-dbz -analyze-patch "
                                 "-report={} -patch={} "
                                 "-omit-no-dbginfo "
                                 "-falcon-use-valuetostring "
                                 "-seg-enable-valuetostring "
                                 "-nworkers=1 -falcon-enable-func={} "
                                 "-execution-mode=digging -set-inc-tactic=smt_tactic "
                                 "{} > {} 2>&1").format(report_file,
                                                        diff_file, peer_file,
                                                        indirect_call,
                                                        input_bc, log_file)
