#include "include/GraphDiffer.h"
#include "include/DriverSpecs.h"
#include "include/NodeHelper.h"
#include "include/UtilsHelper.h"
...
void dumpEnhancedTrace(const EnhancedSEGTrace* trace) {
...
void GraphDiffer::obtainIntraSlicingStage1(set<SEGTraceWithBB> &intraSEGTracesBefore,
  ...
  dbgs() << "2.2 [# After  SEG Traces Stage 1]: " << intraSEGTracesAfter.size()
         << "\n";
  dbgs() << "2.2 [# Backward Visited Stage 1]: " << SEGWrapper->backwardIntraVisited.size()
         << "\n";
  dbgs() << "2.2 [# Forward  Visited Stage 1]: " << SEGWrapper->forwardIntraVisited.size()
         << "\n";
...
void GraphDiffer::obtainIntraSlicingStage2(set<SEGTraceWithBB> &intraSEGTracesBefore,
                                           set<SEGTraceWithBB> &intraSEGTracesAfter) {
  ...
  //        SEGWrapper->obtainIntraEnhancedSlicing(beforeNode, tmpBeforeIntraTrace);
  //      }
  ...
  //        SEGWrapper->obtainIntraEnhancedSlicing(afterNode, tmpAfterIntraTrace);
  //      }
...
void GraphDiffer::obtainIntraSlicingStage3(set<SEGTraceWithBB> &intraSEGTracesBefore,
                                           set<SEGTraceWithBB> &intraSEGTracesAfter) {
  ...
  for (auto item : tmpBeforeIntraTrace) {
    ...
    for (auto cur_item : beforeIntraTraces) {
      if (isTwoEnhancedTraceEq(item, cur_item)) {
        found_exist = true;
        break;
      }
    }
...
//          dbgs() << "[Output Site]: " << *trace1->output_node->usedSite << "\n";
//        }
//
...
//          dbgs() << "[Output Site]: " << *trace2->output_node->usedSite << "\n";
//        }
...
//        outs() << isTwoEnhancedTraceMatch(trace1, trace2) << "\n";
  ...
  for (auto trace1 : afterIntraTraces) {
    if (orderMatchTrace.count(trace1)) {
      continue;
    }
    if (condMatchTrace.count(trace1)) {
      continue;
    }
    if (unchangedIntraTraces.count(trace1)) {
      continue;
    }
    for (auto trace2 : beforeIntraTraces) {
      if (trace1->trace.trace.size() != trace2->trace.trace.size()) {
        continue;
      }
      dbgs() << "2.3 [Compare     Intra Slicings]: \n";
...
void GraphDiffer::classifyInterEnhancedTraces(
  ...
  for (auto afterTrace : afterInterTraces) {
    if (!changedCondInterTraces.count(afterTrace) &&
      ...
      //     DEBUG_WITH_TYPE(
      //         "statistics",
      //         dbgs() << isTwoSEGTraceMatched(before->trace, afterTrace->trace)
      //                << "\n");
      //   }
      // }
...
bool GraphDiffer::isTwoEnhancedTraceEq(EnhancedSEGTrace *trace1,
                                          EnhancedSEGTrace *trace2) {
  if (!isTwoSEGTraceMatched(trace1->trace, trace2->trace)) {
    return false;
  }
...
bool GraphDiffer::isTwoSEGTraceMatchedWithPhi(const vector<SEGObject*> &trace1,
  ...
  for (int i = 0; i < trace1.size(); i++) {
    ...
    if (isa<SEGOperandNode>(node1) && isa<SEGOperandNode>(node2)) {
        ...
        if (isa<SEGPhiNode>(node1) && isa<SEGPhiNode>(node2)) {
          ...
          for (const auto &it : *phiNode1) {
            if (it.ValNode == trace1.at(i - 1)) {
              bbNodes1.insert(it.BB->getRealBlock());
          ...
          for (const auto &it : *phiNode2) {
            if (it.ValNode == trace2.at(i - 1)) {
              bbNodes2.insert(it.BB->getRealBlock());
            } else if (it.ValNode->getLLVMDbgValue() &&
                       trace1[i - 1]->getLLVMDbgValue()) {
              if (it.ValNode->getLLVMDbgValue() ==
                  trace1[i - 1]->getLLVMDbgValue()) {
      ...
      if (node1->getParentGraph()->getBaseFunc()->getName().startswith(
        ...
        if (isa<Constant>(node1->getLLVMDbgValue()) && isa<Constant>(node2->getLLVMDbgValue())) {
          continue;
        }
        matchedNodesBefore.insert({node1, node2});
        matchedNodesAfter.insert({node2, node1});
...
bool GraphDiffer::isTwoSEGTraceMatchedWithoutPhi(const vector<SEGObject *> &trace1,
                                                 const vector<SEGObject *> &trace2) {
...
bool GraphDiffer::isTwoIONodeMatched(EnhancedSEGTrace* trace1, EnhancedSEGTrace* trace2) {
...
bool GraphDiffer::isTwoConditionMatched(
    ConditionNode* cond1,
    ConditionNode* cond2) {
...
bool GraphDiffer::isTwoConditionEqual(
    ConditionNode* cond1,
    ConditionNode* cond2) {
...
// return true
bool GraphDiffer::isTwoConditionMatchedFast(
    ConditionNode* cond1,
    ConditionNode* cond2) {
...
bool GraphDiffer::isTwoConditionMatchedSMT(
    ConditionNode* cond1,
    ConditionNode* cond2) {
...
bool GraphDiffer::isTwoConditionSubMatched(
    ConditionNode* cond1,
    ConditionNode* cond2) {
...
void GraphDiffer::diffTwoPathCondNum(map<Instruction *, CDType> &cond1,
  ...
  for (auto pair1 : cond1) {
    ...
    for (auto pair2 : cond2) {
      ...
      if (matchedIRsAfter.find(pair2.first) !=
              matchedIRsAfter.end() &&
          matchedIRsAfter[pair2.first] != pair1.first) {
        continue;
      }

      if (matchedIRsBefore.find(pair1.first) !=
              matchedIRsBefore.end() &&
...
void GraphDiffer::findHasSubTree(
  ...
  while (!nodeStacks.empty()) {
    ...
    findIfCond2SubTreeCond1(cond1, curNode, curPathCond, matchedSubTree, subMatchedSubTree);
...
void GraphDiffer::findIfCond2SubTreeCond1(
  ...
  while (!nodeStacks.empty()) {
    ...
    else if (isTwoConditionSubMatched(cond2, curNode) && pathCond2 == curPathCond &&
    ...
    else if (isTwoConditionSubMatched(curNode, cond2) && pathCond2 == curPathCond &&
...
map<OutputNode*, pair<int, int>> GraphDiffer::diffTwoOrder(map<OutputNode*, int> &site2order1,
...
ConditionNode* GraphDiffer::diffTwoConditions(
    ConditionNode* condMap1,
  ...
  //        dbgs() << myValueToString1(cond1->getLLVMDbgValue()) << ":"
  //               << myValueToString1(cond2->getLLVMDbgValue()) << "\n");
  //        dbgs() << isTwoSEGNodeMatch(cond1, cond2) << "\n");
  //        break;
  //      }
  //
  ...
  //        if (!isTwoSEGNodeMatch(cond1, cond2)) {
  //          continue;
  //        } else {
  //          nodeMaps.insert({cond1, cond2});
  //          nodeMaps.insert({cond2, cond2});
  ...
  //    dbgs() << valueToString(node1->getLLVMDbgValue()) << ":" << valueToString(node2->getLLVMDbgValue()) << "\n");
  //  }
  //  ConditionTree::diffCondition(condMap1, condMap2);
  ...
  DEBUG_WITH_TYPE("condition",  dbgs() << "\nMatched Cond Node " << matchedCondNodes.size() << "\n");
  DEBUG_WITH_TYPE("condition",  dbgs() << "Matched And Cond Node " << matchedAndNode << "\n");
  DEBUG_WITH_TYPE("condition",  dbgs() << "Matched Single Cond Node " << matchedSingleNode << "\n");
