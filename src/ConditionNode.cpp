...
ConditionNode *ConditionNode::distribute(ConditionNode * node,
...
ConditionNode *ConditionNode::distributeANDoverOR(
    ConditionNode * node, size_t index, int depth, bool& changed) {
  ...
  for (ConditionNode *orChild : orNode->children) {
    ...
    newAndNode->addChild(
        orChild); // Recursively distribute subchildren of OR
...
ConditionNode *ConditionNode::distributeORoverAND(
    ConditionNode * node, size_t index, int depth, bool& changed) {
  if (depth > 5) {
    return node;
  }
  ...
  // Return the newly created AND node which has distributed the original AND over OR
...
bool isInvalidNode(ConditionNode* node) {return node->children.empty()
                                                 && (node->type == NODE_AND ||
                                                     node->type == NODE_OR ||
                                                     node->type == NODE_NOT);}
  ...
  if (this->type == NODE_NOT) {
    ...
    if (current->type == NODE_VAR) {
      if (notCount % 2 == 0) {
        ...
        this->children[0] = current; // Keep the last NOT's child as the current variable
...
bool isSubVector(const vector<ConditionNode*>& a, const vector<ConditionNode*>& b) {
...
bool ConditionNode::isAbsorptionLaw(ConditionNode *a, vector<ConditionNode*>& b) {
  // a is (A and B)
  // b is [A]
...
ConditionNode* ConditionTree::parseFromString(string dumped_str, EnhancedSEGWrapper* SEGWrapper, set<SEGNodeBase*> nodeSet) {
  ...
  while (getline(iss, line)) {
    ...
    if (node_type == NODE_VAR) {
      string value_str = node_type_str.substr(node_type_str.find("%"),
                                              node_type_str.find(")") - node_type_str.find("%"));
...
ConditionNode *ConditionTree::simplifyCondition(EnhancedSEGWrapper *SEGWrapper, ConditionNode *node) {
  set<SEGNodeBase*> allSEGNode = node->obtainNodes();

  string command = "python3 /Users/harperchen/PycharmProjects/pythonProject9/main.py simplify " + createArgFile(node->dump());
  string outputExpr = readFileContents(execCommand(command));
  return parseFromString(outputExpr, SEGWrapper, allSEGNode);
}

ConditionNode *ConditionTree::diffCondition(ConditionNode *node1, ConditionNode* node2) {
