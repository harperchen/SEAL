...
struct seg_patch_cmp {
  ...
  seg_patch_cmp() : instance(LLVMValueIndex::get()) {}
  ...
  bool operator()(const SEGObject *A, const SEGObject *B) const {
    ...
    StringRef funcSEGA = "";
    StringRef funcSEGB = "";
    if (A && A->getParentGraph() && A->getParentGraph()->getBaseFunc() && A->getParentGraph()->getBaseFunc()->hasName()) {
      funcSEGA = A->getParentGraph()->getBaseFunc()->getName();
    }
    if (B && B->getParentGraph() && B->getParentGraph()->getBaseFunc() && B->getParentGraph()->getBaseFunc()->hasName()) {
