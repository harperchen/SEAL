...
class PatchParser:
    ...
    def find_similar_bugs(self, workdir):
        reasons = []
        diff_file = os.path.abspath(os.path.join(workdir, 'diff.txt'))
        report_file = os.path.abspath(os.path.join(workdir, 'bug.json'))
        input_bc = os.path.abspath(os.path.join(workdir, 'merge.bc'))
        log_file = os.path.abspath(os.path.join(workdir, 'patch.log'))
...
if __name__ == "__main__":
    ...
    for idx, item in df.iterrows():
        ...
        process_patch(analyze_idx, parser, item, workdir, is_failed)
        analyze_idx += 1
