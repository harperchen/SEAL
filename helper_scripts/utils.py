...
# cclang-12 -Wp,-MMD,arch/x86/kernel/acpi/.wakeup_64.o.d ...  -c -o arch/x86/kernel/acpi/wakeup_64.o arch/x86/kernel/acpi/wakeup_64.S
def handle_cc(linux_dir, cmd_o: str):
    if 'BUILD_STR(s)=$(pound)s' in cmd_o:
        return '', ''

    res = cmd_o[:cmd_o.find(" -I")]
    res += " -isystem /usr/lib/clang/12.0.0/include"
...
# objcopy -O elf32-x86-64 arch/x86/entry/vdso/vsgx.o arch/x86/entry/vdso/vsgx-x32.o
...
def handle_ld(linux_dir, cmd_ld: str):
    res, target = '', ''
    # ld  -m elf_x86_64 -z noexecstack     -r -o drivers/mtd/tests/mtd_pagetest.o @drivers/mtd/tests/mtd_pagetest.mod
    if '@' in cmd_ld:
...
# strip --strip-debug -o drivers/firmware/efi/libstub/alignedmem.stub.o drivers/firmware/efi/libstub/alignedmem.o; if objdump ...
...
# rm -f drivers/soc/microchip/built-in.a;  printf "drivers/soc/microchip/%s " mpfs-sys-controller.o | xargs ar cDPrST drivers/soc/microchip/built-in.a
