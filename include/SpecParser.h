...
struct BugSpecification {
  ...
  ConstExpr *constExpr = nullptr; // more precise constraint
  vector<Function *> indirects;
  bool fastMode = false;

  BugSpecification(specType type, vector<Function *> indirects, bool fastMode)
      : type(type), indirects(indirects), fastMode(fastMode){};

  BugSpecification(specType type)
      : type(type) {};
};
...
struct SingleSrcSingleSinkSpec : BugSpecification {
  ...
  SingleSrcSingleSinkSpec(InputNode *inputNode, OutputNode *outputNode, bool isBuggy)
      : BugSpecification(BS_SingleSrcSingleSink),
        inputNode(inputNode), outputNode(outputNode), isBuggy(isBuggy){};
};
...
struct SingleSrcMultiSinkSpec : BugSpecification {
  ...
  SingleSrcMultiSinkSpec(InputNode *inputNode, vector<OutputNode *> outputs,
                         map<OutputNode *, pair<int, int>> output2Order)
      : BugSpecification(BS_SingleSrcMultiSink),
        inputNode(inputNode), outputNodes(outputs), output2Order(output2Order) {};
};
