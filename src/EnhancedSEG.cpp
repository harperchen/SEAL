#include "include/EnhancedSEG.h"
#include "include/ConditionNode.h"
#include "include/ValueHelper.h"
...
EnhancedSEGWrapper::EnhancedSEGWrapper(
    ...
    CBCallGraphWrapperPass *pCallGraphWrapper, ControlDependenceAnalysis *pCDGs,
    CFGReachabilityAnalysis *pCRA, DomTreePass *pDT) {

  M = pM;
  SEGBuilder = pSEGBuilder;
  SEGSolver = pSEGSolver;
  CBCG = pCallGraphWrapper;
  DIA = pDIA;
  CRA = pCRA;
  CDGs = pCDGs;
  DT = pDT;
...
void EnhancedSEGWrapper::collectRelatedBBs(
    vector<SEGObject *> &trace, int index,
...
//        DEBUG_WITH_TYPE("condition",  dbgs() << *loadNode->getMatchingCondition(node) << "\n");
//      }
//    }
//    for (auto it = loadNode->use_site_begin(); it != loadNode->use_site_end(); it++) {
//      DEBUG_WITH_TYPE("condition",  dbgs() << **it << "\n");
//    }
  }
  else if (auto *pseudoInputNode = dyn_cast<SEGCallSitePseudoInputNode>(trace[index])) {
    ...
    if (auto *ins = dyn_cast<Instruction>(trace[index]->getLLVMDbgValue())) {
      ...
      for (const auto &it : *phiNode) {
        if (it.ValNode == trace[index] && it.ValNode->getLLVMDbgValue() &&
          ...
          trace[index]->setParentBasicBlock(it.BB->getRealBlock());
        }
      }
      ...
      for (const auto & it : *phiNode) {
        if (it.ValNode == trace[index - 1]) {
          if (it.BB) {
            if (curbbOnTraces.empty() ||
                curbbOnTraces.back() != it.BB->getRealBlock()) {
              curbbOnTraces.push_back(it.BB->getRealBlock());
...
std::vector<T> concatenate(const std::vector<T>& v1, const std::vector<T>& v2) {
  ...
  result.insert(result.end(), v2.begin(), v2.end());  // Append all elements from v2
  return result;
}

void EnhancedSEGWrapper::collectBBsToEntry(EnhancedSEGTrace *trace,
  ...
  BasicBlock *startBB = &bbOnTraces.front()->getParent()->getEntryBlock();
...
bool EnhancedSEGWrapper::checkCurPathFeasibility(vector<pair<BasicBlock *, CDType>> path) {
...
ConditionNode* EnhancedSEGWrapper::path2IOCondition(vector<pair<BasicBlock *, CDType>> path,
...
bool EnhancedSEGWrapper::checkifICMPIO(ICmpInst* iCmpInst, vector<SEGObject*> &guardedTrace) {
  auto vf_start = chrono::high_resolution_clock::now();
...
void EnhancedSEGWrapper::canFindInput(vector<SEGObject *> trace,
                                      set<InputNode*> &inputNodes, bool intra) {
  if (trace.empty()) {
    return;
  }
  int startIndex = -1;
  ...
  if (startNode) {
    ...
    if (auto *argNode = dyn_cast<SEGArgumentNode>(startNode)) {
      ...
      if (intra || (!intra && isIndirectCall(func))) {
        ...
        auto input =
            new IndirectArgNode(pathFuncName, argNode->getDescription());
    ...
    // TODO: add arg of API as input
      ...
      // if error code, verify if it is caused by API failure
          ...
          if (auto *phiNode = dyn_cast<SEGPhiNode>(trace[startIndex + 1])) {
            ...
            for (const auto &it : *phiNode) {
              if (it.ValNode == startNode) {
                findLastIcmp(it.BB->getRealBlock(), icmpInsts);
        ...
        if (intra || (!intra && isIndirectCall(parent_func))) {
          ...
          auto input = new IndirectArgNode(pathFuncName, arg->getName());
          input->usedNode = startNode;
          inputNodes.insert(input);
        }
...
SMTExpr EnhancedSEGWrapper::condDataDepToExpr(ConditionNode* curNode,
...
void EnhancedSEGWrapper::condNode2FlowInter(set<SEGNodeBase*> condNodes,
                                            map<SEGNodeBase *, set<vector<SEGObject *>>>& localCond2ValueFlows) {
...
void EnhancedSEGWrapper::condNode2FlowIntra(set<SEGNodeBase*> condNodes,
                                            map<SEGNodeBase *, set<vector<SEGObject *>>>& localCond2ValueFlows) {
        ...
        if (otherIcmp->getPredicate() == llvm::CmpInst::ICMP_EQ) {
          if (isTwoCondConverse) {
            ...
            if (curCond->type != NODE_NOT) {
              DEBUG_WITH_TYPE("condition",  dbgs() << "\n[1 Reduce 2 Const 1] " << curCond->dump() << "\n");
              DEBUG_WITH_TYPE("condition",  dbgs() << "[1 Reduce 2 Const 2] " << otherCond->dump() << "\n");
...
bool EnhancedSEGWrapper::isConditionConflict(ConditionNode* curCond, ConditionNode* otherCond) {
  ...
  //
  //  DEBUG_WITH_TYPE("condition",  dbgs() << "Verifying LLVM Value 1" << *llvmValue1 << "\n");
  //  DEBUG_WITH_TYPE("condition",  dbgs() << "Verifying LLVM Value 2" << *llvmValue2 << "\n");
  if (curSEG->findNode(otherIcmp->getOperand(0)) &&
      curSEG->findNode(curIcmp->getOperand(0)) &&
      isTwoSEGNodeValueEqual(curSEG->findNode(curIcmp->getOperand(0)),
                             curSEG->findNode(otherIcmp->getOperand(0)))) {
    ...
    if (curSEG->findNode(curIcmp->getOperand(1)) &&
      ...
      if (otherIcmp->getPredicate() == curIcmp->getPredicate()) {
        if (isTwoCondConverse) {
          DEBUG_WITH_TYPE("condition",  dbgs() << "\n[Conflict Node 1 Diff] " << curCond->dump() << "\n");
          DEBUG_WITH_TYPE("condition",  dbgs() << "[Conflict Node 2 Diff] " << otherCond->dump() << "\n");
        }
        if (isTwoCondConverse) {
...
// here, we consider the case where p = q, p = not q, p => q or q => p
bool EnhancedSEGWrapper::isConditionMerge(ConditionNode* curCond, ConditionNode* otherCond) {
  ...
  if (curSEG->findNode(otherIcmp->getOperand(0)) &&
    ...
    if (curSEG->findNode(curIcmp->getOperand(1)) &&
      ...
      if (otherIcmp->getPredicate() == curIcmp->getPredicate()) {
        if (isTwoCondSame) {
          DEBUG_WITH_TYPE("condition",  dbgs() << "\n[Merge Node 1 Diff] " << curCond->dump() << "\n");
          DEBUG_WITH_TYPE("condition",  dbgs() << "[Merge Node 2 Diff] " << otherCond->dump() << "\n");
        }
         if (isTwoCondSame) {
...
void EnhancedSEGWrapper::intraValueFlow(
    SEGNodeBase *criterion, set<SEGTraceWithBB> &intraTraces) {
  vector<SEGObject *> curTrace;
  ...
  vf_start = chrono::high_resolution_clock::now();
//   dbgs() << "After intra, backward num: " << backwardTraces.size() << " " << *criterion << "\n";
//   dbgs() << "After intra, forward num: " << forwardTraces.size() << " " << *criterion << "\n";
        ...
        if (isa<ConstantInt>(cur_incoming->getLLVMDbgValue())) {
          auto newConstNode = new SEGSimpleOperandNode(
              ...
              SEGBuilder->getSymbolicExprGraph(
                  phiNode->getIncomeNode(i)->BB->getParent()->getRealFunc()));
...
void EnhancedSEGWrapper::funcCallLowerTracer(
    Function *func, vector<Function *> &curTrace,
...
void EnhancedSEGWrapper::funcCallUpperTracer(
    Function *func, vector<Function *> &curTrace,
    set<vector<Function *>> &traces) {
  curTrace.push_back(func);
...
bool EnhancedSEGWrapper::needBackward(SEGNodeBase *node) {
  ...
  if (isa<SEGArgumentNode>(node)) {
    if (isIndirectCall(node->getParentGraph()->getBaseFunc())) {
      ...
      //      dbgs() << "Meet Indirect Call " << node->getParentGraph()->getBaseFunc()->getName() << "\n");
      return false;
    }
    return true;
  } else if (auto *CSONode = dyn_cast<SEGCallSiteOutputNode>(node)) { // into caller
    auto callee = CSONode->getCallSite()->getCalledFunction();
...
void EnhancedSEGWrapper::computeCallGraph() {
  ...
  for (Function &F : *M) { // key: caller, value: set of callees
    auto node = CBCG->getOrInsertFunction(&F);
    ...
    for (auto & it : *node) {
      ...
      if (callGraphCycle.find(&F) == callGraphCycle.end()) {
        set<Function *> children;
        callGraphCycle[&F] = children;
      }
      callGraphCycle[&F].insert(callee);
    }
  }
...
bool EnhancedSEGWrapper::isKernelOrCommonAPI(StringRef funcName) {
  ...
  set<string> notKernelAPI = {"__dynamic_dev_dbg", "_printk",
                              "_dev_err", "llvm.memcpy.p0i8.p0i8.i64",
  ...
  for (BasicBlock &B : *F) {
    for (Instruction &I : B) {
      ...
      if (start_line == -1) {
        ...
        if (source_file == "" &&
            DIA->getSrcFile(&I).find("include/") != string::npos) {
          source_file = DIA->getSrcFile(&I);
          source_file = source_file.substr(source_file.find("include/"));
        }
        if (source_file == "" &&
...
// some rare cases that two SEG nodes essentially always carry on the same value,
// but are different llvm value or different SEG nodes,
...
bool EnhancedSEGWrapper::isTwoSEGNodeValueEqual(SEGNodeBase *node1,
  ...
  vector<vector<SEGObject *>> intraTraceNodeVec1(intraTraceNodeVec1Set.begin(), intraTraceNodeVec1Set.end()),
      intraTraceNodeVec2(intraTraceNodeVec2Set.begin(), intraTraceNodeVec2Set.end());
...
void EnhancedSEGWrapper::updateTraceOrder(map<SEGNodeBase*, set<EnhancedSEGTrace*>> &groupedTraces) {
...
void EnhancedSEGWrapper::findLastIcmp(BasicBlock *bb, set<ICmpInst *> &icmpInsts) {
...
void EnhancedSEGWrapper::canFindOutput(const vector<SEGObject *> trace,
                               set<OutputNode *> &outputNodes, bool isBenign, bool intra) {
  if (trace.empty()) {
    return;
  }
  int endIndex = -1;
